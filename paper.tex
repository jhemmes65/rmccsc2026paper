...
The instructional model we use to teach design patterns involves three sequential steps, each building on the previous and serving a distinct purpose.

\subsection{Step 1: Examine a Working Code Example}

To begin, students are provided a working example in code that implements a common design pattern. They are not told yet what pattern it is or what design problem is being solved, as this step is intended to serve as a discovery process. Students are initially asked to study the code and explain what they think it does. They are also asked to identify various elements of the code, e.g., concrete or abstract classes, interfaces, inheritance, polymorphism, abstractions, and anything else that stands out, whether something they are unfamiliar with or unusual. When covering design patterns, students already have the prerequisite familiarity with object-oriented concepts and object-oriented programming.
